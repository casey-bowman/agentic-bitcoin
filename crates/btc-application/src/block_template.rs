...
use btc_domain::consensus::{ConsensusParams, MAX_BLOCK_WEIGHT};
use btc_domain::primitives::{
    Amount, Block, BlockHeader, Hash256, Transaction, TxOut,
};
use btc_domain::script::Script;
use btc_ports::{BlockTemplate, BlockTemplateProvider, ChainStateStore, MempoolPort};
use std::error::Error;
use std::sync::Arc;
...
mod tests {
    use super::*;
    use btc_domain::primitives::{BlockHash, OutPoint, TxIn, Txid};
    ...
    impl ChainStateStore for MockChainState {
        ...
        async fn get_utxo_set_info(
            &self,
        ) -> Result<btc_ports::UtxoSetInfo, Box<dyn Error + Send + Sync>> {
