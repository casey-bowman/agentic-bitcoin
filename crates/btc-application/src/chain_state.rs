...
impl ChainState {
    ...
    pub async fn flush_to_store(
        ...
        let utxo_adds: Vec<(btc_domain::primitives::Txid, u32, UtxoEntry)> = self
            .utxo_set
            .iter()
            .map(|(outpoint, entry)| {
                (
    ...
    pub async fn flush_block_delta(
        ...
        let adds: Vec<(btc_domain::primitives::Txid, u32, UtxoEntry)> = result
            .created
            .iter()
            .map(|(outpoint, entry)| {
                (
        ...
        let removes: Vec<(btc_domain::primitives::Txid, u32)> = result
            .spent
            .keys()
...
mod tests {
    ...
    async fn test_load_tip_from_empty_store() {
        let store = btc_adapters::storage::InMemoryChainStateStore::new();
