...
use btc_domain::policy::limits::MempoolLimits;
use btc_domain::primitives::{Amount, Transaction};
use btc_domain::script::{verify_script_with_witness, ScriptFlags};
use btc_ports::{ChainStateStore, MempoolPort};
use std::sync::Arc;
...
pub struct AcceptResult {
    /// The transaction ID.
    pub txid: btc_domain::primitives::Txid,
...
mod tests {
    ...
    use btc_domain::primitives::{OutPoint, TxIn, TxOut, Txid};
    use btc_domain::Script;
    use btc_ports::{MempoolEntry, MempoolInfo, UtxoEntry};
    ...
    impl ChainStateStore for MockChainState {
        ...
        async fn get_best_chain_tip(
            &self,
        ) -> Result<(btc_domain::primitives::BlockHash, u32), Box<dyn std::error::Error + Send + Sync>>
        {
            Ok((btc_domain::primitives::BlockHash::zero(), 0))
        }

        async fn write_chain_tip(
            &self,
            _hash: btc_domain::primitives::BlockHash,
            _height: u32,
        ) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
            Ok(())
        }
        async fn get_utxo_set_info(
            &self,
        ) -> Result<btc_ports::UtxoSetInfo, Box<dyn std::error::Error + Send + Sync>> {
            Ok(btc_ports::UtxoSetInfo {
                txout_count: 0,
                total_amount: btc_domain::primitives::Amount::from_sat(0),
                best_block: btc_domain::primitives::BlockHash::zero(),
                height: 0,
            })
        }
    }
    ...
    async fn test_accept_valid_transaction() {
        ...
        let funding_txid = Txid::from_hash(btc_domain::primitives::Hash256::from_bytes([0x01; 32]));
        chain_state
    ...
    async fn test_reject_outputs_exceed_inputs() {
        ...
        let funding_txid = Txid::from_hash(btc_domain::primitives::Hash256::from_bytes([0x02; 32]));
        chain_state
    ...
    async fn test_reject_dust_output() {
        ...
        let funding_txid = Txid::from_hash(btc_domain::primitives::Hash256::from_bytes([0x03; 32]));
        chain_state
            .add_utxo(funding_txid, 0, make_funding_utxo(100_000))
            .await;
    ...
    async fn test_fee_calculation() {
        ...
        let funding_txid = Txid::from_hash(btc_domain::primitives::Hash256::from_bytes([0x04; 32]));
        chain_state
    ...
    async fn test_duplicate_rejection() {
        ...
        let funding_txid = Txid::from_hash(btc_domain::primitives::Hash256::from_bytes([0x05; 32]));
        chain_state
            .add_utxo(funding_txid, 0, make_funding_utxo(100_000))
            .await;
