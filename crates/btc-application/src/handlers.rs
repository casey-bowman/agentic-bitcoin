...
use btc_ports::{ChainStateStore, RpcHandler};
        ...
        match method {
            ...
            "getblockhash" => {
                let height = _params.get(0)
                    .and_then(|v| v.as_u64())
                    .ok_or_else(|| btc_ports::RpcError::invalid_params("missing height"))?
                    as u32;
                ...
                match idx.get_hash_at_height(height) {
                    ...
                    None => Err(btc_ports::RpcError::invalid_params(
            ...
            "getblock" => {
                let hash_hex = _params.get(0)
                    .and_then(|v| v.as_str())
                    .ok_or_else(|| btc_ports::RpcError::invalid_params("missing blockhash"))?;
                let verbosity = _params.get(1)
                    .and_then(|v| v.as_u64())
                    .unwrap_or(1);

                let hash = btc_domain::primitives::BlockHash::from_hex(hash_hex)
                    .ok_or_else(|| btc_ports::RpcError::invalid_params("invalid blockhash"))?;

                let block = self.blockchain
                    .get_block(&hash)
                    .await
                    .map_err(|e| btc_ports::RpcError::internal_error(e))?
                    .ok_or_else(|| btc_ports::RpcError {
                        code: -5,
            ...
            "sendrawtransaction" => {
                ...
                let hex_str = _params.get(0)
                    .and_then(|v| v.as_str())
                    .ok_or_else(|| btc_ports::RpcError::invalid_params("missing hex string"))?;

                let tx_bytes = hex::decode(hex_str).map_err(|_| {
                    btc_ports::RpcError::invalid_params("invalid hex encoding")
                })?;

                let (tx, _) = btc_domain::primitives::Transaction::deserialize(&tx_bytes)
                    .map_err(|e| btc_ports::RpcError::invalid_params(
            ...
            "getrawtransaction" => {
                ...
                let txid_hex = _params.get(0)
                    .and_then(|v| v.as_str())
                    .ok_or_else(|| btc_ports::RpcError::invalid_params("missing txid"))?;
                let verbose = _params.get(1)
                    .and_then(|v| v.as_u64())
                    .unwrap_or(0);

                let txid = btc_domain::primitives::Txid::from_hex(txid_hex)
                    .ok_or_else(|| btc_ports::RpcError::invalid_params("invalid txid"))?;
...
mod tests {
    ...
    async fn test_gettxoutsetinfo_empty() {
        ...
        let chain_state = Arc::new(btc_adapters::storage::InMemoryChainStateStore::new());

        // Add a UTXO
        use btc_domain::primitives::Amount;
        let txid = Txid::zero();
        let entry = btc_ports::UtxoEntry {
        ...
        chain_state
            .write_chain_tip(
                btc_domain::primitives::BlockHash::zero(),
                5,
            )
            .await
            .unwrap();
    ...
    fn test_block_index_height_lookup() {
        use crate::block_index::BlockIndex;
        use btc_domain::primitives::{BlockHash, Hash256, BlockHeader};
